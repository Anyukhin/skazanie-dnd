...
export type TavernRoundCard = {
  ...
  target: number
  /**
   * Почему раунд уже не доиграть, или `null` — «садись и отвечай».
   *
   * Считает сервер (`tavernRoundUnanswerableReason`): своей арифметики чужой
   * кассы у доски быть не должно, иначе панель обещала бы возврат там, где
   * ставку заберёт соперник. Встать можно всегда — но от живой кости это сдача,
   * и стоит она ровно как проигрыш.
   *
   * Возврат ставки идёт **не** по этому полю, а по виновнику тупика, и совпадает
   * с ним только на `patron-ejected`: за дверь героя выставляет заведение.
   * Разорить соседа может только сам отряд, поэтому `opponent-broke` ставку со
   * стола не возвращает — иначе открытие раунда было бы игрой без риска.
   *
   * `hero-broke` из набора убран: ставка уплачена при открытии раунда, поэтому
   * бедность отвечать не мешает и поводом тупика быть не может.
   */
  unanswerable_reason?: 'opponent-broke' | 'patron-ejected' | null
}
