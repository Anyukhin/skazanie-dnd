...
export type TavernRoundCard = {
   ...
   * решает, пускать ли героя из-за стола: своей арифметики чужой кассы и своего
   * кошелька у доски здесь быть не должно, иначе кнопка «встать» горела бы там,
   * где придёт отказ. Кость на столе — это уже сделанная ставка.
   */
  unanswerable_reason?: 'opponent-broke' | 'hero-broke' | 'patron-ejected' | null
}
