...
const FEAT_DEFINITIONS = [
  ...
  ['durable', 'Крепкий', 'Durable', 'Отдых и Кости Хитов восстанавливают больше здоровья.'],
  ...
  ['resilient', 'Стойкий', 'Resilient', 'Одна характеристика растёт, а её спасбросок становится привычным.', [], { ability: { type: 'ability', options: [...ABILITY_IDS] } }],
  ...
  ['tough', 'Устойчивый', 'Tough', 'Каждый уровень даёт ещё два максимальных хита.'],
...
function freezeDefinition([id, name, englishName, summary, prerequisites = [], choiceSchema]) {
  return Object.freeze({
    ...
    choice_schema: clone(choiceSchema ?? {}), mechanics_status: 'partial',
  })
}
...
function addAbilityIncrease(benefits, ability, context) {
  ...
  if (abilityScore(context, ability) >= 20) return { ok: false, code: 'ABILITY_MAX_REACHED', reason: 'Значение характеристики уже достигло максимума 20' }
