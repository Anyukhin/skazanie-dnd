 ...
 *   уговора зверя;
 * - **благословение не складывается само с собой**: пока висит первое, второго
 *   не дают ни у алтаря, ни у жреца (`BLESSING_ALREADY_ACTIVE`). Одних суток
 *   для этого мало: слот открывается через 1440 минут, а состояние снимается
 *   только продолжительным отдыхом или первым ударом — сутки пути без ночёвки и
 *   без боя открывали окно, в котором треба брала золотой и не давала ничего;
 * - **жрец опознаётся ролью, и только ролью**: тег `faction:` есть далеко не у
 *   каждого NPC, а роль в профиле есть всегда, и именно она видна игроку на
 *   токене. Тег в опознание не входит: `faction:priesthood-of-dawn` на кузнеце
 *   называет его прихожанином храма, а не служителем.
 */
 ...
 *
 * Основы закреплены с обеих сторон там, где голая основа мажет. Слева: `монах`
 * без границы сидел бы в «мономахе», `инок` — в «поединке». Справа и слева
 * сразу — `свяще`: голая основа объявляла служителем «освященного стража» и
 * «посвященного», то есть благословлять мог посвящённый культист. Поэтому от
 * основы остались только существительные — «священник», «священница»,
 * «священнослужитель», — а прилагательные «священный» и «освященный» отпали
 * вместе с ней. Кириллица для `\b` буквой не считается, поэтому границы просят
 * ретроспективной проверки, а не `\b`.
 *
 * Отдельными строками стоит то, что ретроспективная проверка резала вместе с
 * чужими словами или чего в списке не было вовсе: «иеромонах» (перед `монах`
 * стоит буква), «монашка» (основа другая) и двусловные роли — «хранитель
 * святилища», «служительница богов». Модель их сочиняет, а храм от них
 * отказывался.
 */
const PRIEST_PATTERN = /жре[цч]|жриц|(?<![а-яё])священн(?:ик|иц|ослужител)|иерей|иеромонах|настоятел|аббат|клирик|(?<![а-яё])монах|(?<![а-яё])монашк|(?<![а-яё])инок(?![а-яё])|проповедн|хранител[а-яё]{0,6} святилищ|служител[а-яё]{0,6} (?:храм|святилищ|бог)|priest|cleric|abbot|(?<![a-zа-яё])monk(?![a-zа-яё])|acolyte|послушни/iu
...
/**
 * Служитель ли это. Читается **только** роль профиля — то же слово, что видит
 * игрок на токене.
 *
 * Теги в проверяемую строку не входят, и это не упущение: `faction:` называет
 * принадлежность, а не занятие. Пока теги склеивались с ролью в одну строку,
 * `faction:priesthood-of-dawn` делал служителем кузнеца, а тег со словом
 * «свяще» — нищего; благословлять при этом сможет тот, у кого в профиле нет ни
 * одного жреческого слова, зато есть тег храма.
 */
export function isPriestProfile(npc = {}) {
  return PRIEST_PATTERN.test(text(npc?.role, 160))
}
