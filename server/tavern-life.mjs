 ...
 *    стол. Дальше она возвращается ровно четырьмя путями: выигрыш (ставка и
 *    приз), ничья и разоблачение шулера (ставка), отмена раунда по разорению
 *    соперника (ставка), уход выставленного за дверь (ставка — выставили не
 *    его волей). Проигрыш, поимка за руку и добровольный уход из живого раунда
 *    не возвращают ничего: последний — это сдача, и стоит она ровно столько же,
 *    сколько проигрыш.
 *
 *    Следствие важнее самой правки: «своих денег не хватает» перестаёт быть
 *    поводом тупика **по построению**. Ставка уже уплачена, отвечать можно
 *    всегда, и `tavernRoundUnanswerableReason` знает теперь только два повода —
 *    оба чужие (соперник разорился, героя выставили за дверь).
 * 7. **Раунд закрывается не только ответом.** Обыграть соседа дочиста может
 *    товарищ по отряду, и тогда банк соперник уже не потянет; хозяин заведения
 *    может выставить героя за дверь между чужим броском и его ответом. Оба
 *    случая упираются в проверки ответа, поэтому у раунда есть и отмена по
 *    команде, и автозакрытие в тот момент, когда чужая касса опустела. В обоих
 *    эскроу возвращается: тупик устроил не герой.
 *
 ...
 *   Вместе со счётом уходит и открытый раунд, а с ним — зарезервированная
 *   ставка: вышел из зала, не ответив на кость, — ставка осталась на столе.
 *   Это тот же уход из живого раунда, только дверью, и стоит он того же. Тихой
 *   пропажи денег здесь нет: кошелёк похудел на виду, событием
 *   `TavernDiceRoundOpened`, ещё когда кость легла на стол.
 */
 ...
 * (тогда банк соседу нечем закрыть) или самого героя могли выставить за дверь
 * (тогда с ним не садятся). Оба случая упираются в проверки
 * `AnswerTavernDiceRound`, и без команды «встать из-за стола» из них не было
 * выхода.
 *
 * Разрешена команда при этом **всегда**, и запрета ей больше не нужно: ставка
 * уже уплачена при открытии раунда, поэтому уход от живой кости ничем не
 * отличается от проигрыша — эскроу остаётся сопернику. Из тупика (`opponent-broke`,
 * `patron-ejected` — `tavernRoundUnanswerableReason`) та же команда возвращает
 * ставку: тупик устроил не герой.
 */
...
/**
 * Почему открытый раунд уже не доиграть — или `null`, если доиграть его можно.
 *
 * Ответ здесь один на всех, и это главное в этой функции. По нему движок решает,
 * вернуть ли ставку вставшему из-за стола (`LeaveTavernDiceRound`,
 * `server/rules-engine.mjs`), по нему же гаснут кнопки ответа на доске и по нему
 * же подсказка зовёт встать, а не бросать. Второй ответ на этот вопрос означал
 * бы возврат там, где движок его не даёт, — или горящую кнопку ответа под
 * запретом входа.
 *
 * Поводов **два**, и оба чужие: соперник спустил всё (тогда банк ему не
 * закрыть) и героя выставили за дверь (тогда с ним не садятся вовсе). Первый —
 * это дословно проверка `AnswerTavernDiceRound`, второй — запрет входа, из-под
 * которого ответа тоже нет.
 *
 * Третьего повода — «своих денег на ставку уже нет» — здесь больше нет, и это
 * не забывчивость, а следствие эскроу. Ставка уходит из кошелька в тот момент,
 * когда кость легла на стол, поэтому ответ не стоит герою ни медяка и бедность
 * ему не мешает. Пока ставка лежала в кармане до расчёта, этот повод был
 * бесплатным выходом из проигрышного раунда: кружка эля за 4 мм роняла кошелёк
 * ниже ставки, и герой вставал из-за стола даром, уже увидев чужие 19.
 *
 * `null` здесь — это «садись и отвечай»: встать из-за стола герою по-прежнему
 * можно, но это сдача, и ставку он оставляет сопернику.
 */
export function tavernRoundUnanswerableReason(state = {}, round = null) {
  const safe = safeRound(round)
  if (!safe) return null
  if (tavernEjected(state, safe.hero_id)) return 'patron-ejected'
  if (tavernGamblerPurseFor(state, safe.npc_id) < safe.stake_cp) return 'opponent-broke'
  return null
}
...
export function tavernForViewer(state = {}, viewer = {}) {
  ...
  return {
    ...
    // Чужая кость соседа по отряду кнопки игроку не блокирует, а к своей
    // приложено то, ради чего доска вообще что-то решает: можно ли ещё
    // ответить и вернут ли ставку тому, кто встал. Считает это сервер и одним
    // ответом на всех — иначе доска обещала бы возврат там, где движок отдаст
    // ставку сопернику.
    round: publicRound && { ...publicRound, unanswerable_reason: tavernRoundUnanswerableReason(state, round) },
 ...
 * встающий из-за стола оставляет сопернику зарезервированную ставку). Событие
 * без `npc_purse_after_cp` кассу не трогает вовсе — так закрытие тупика с
 * возвратом и отличается от сдачи.
 */
...
export function applyTavernEvent(input, event, state = {}) {
  ...
  // пока герой думал. Ставка при этом уже уплачена, и куда она уехала — решил
  // движок: тупик возвращает её герою, добровольный уход отдаёт сопернику.
  // Касса соседа поэтому переносится тем же путём, что и на расчёте, — числом
  // из события.
