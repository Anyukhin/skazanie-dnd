 ...
 * а про чужой, и показать ей нечего, кроме чужой кости.
 *
 * Порядок закреплён по идентификатору героя, чтобы у двух открытых раундов был
 * один и тот же ответ, — и берётся он у `tavernOpenRounds`, а не собирается
 * заново. Второй обход посетителей значил бы второй порядок, который однажды
 * разойдётся с первым.
 */
function anyOpenRound(state = {}) {
  return tavernOpenRounds(state)[0] ?? null
}

/**
 * Самая крупная ставка, которую этот сосед может закрыть **сейчас**. Уезжает
 * игроку в карточке заведения: отказ движка должен быть виден кнопкой, а не
 * приходить ошибкой после клика.
 *
 ...
 *
 * Ноль поэтому значит «свободных денег нет», а не «сосед на мели»: у него может
 * лежать полная касса, расписанная по чужим раундам. Разницы в этом числе не
 * видно, и доска её не выдумывает — настоящую причину называет отказ движка,
 * который считает и кассу, и закреплённое (`TAVERN_OPPONENT_BROKE`).
 */
