 ...
 * 6. **Из-за стола встают только те, кому отвечать нечем.** Раунд открывается
 *    броском соперника, а закрывается не только ответом: обыграть соседа
 *    дочиста может товарищ по отряду, и тогда банк соперник уже не потянет;
 *    деньги героя между фазами тоже могли уйти другой командой. Оба случая
 *    упираются в проверки ответа, поэтому у раунда есть и отмена по команде, и
 *    автозакрытие в тот момент, когда чужая касса опустела.
 *
 *    Но **только** эти случаи, и это ограничение важнее самой команды. Число
 *    соперника видно до решения — оно и в проекции, и в событии, и на панели
 *    доски прямым текстом. Бесплатный выход из раунда после чужого броска
 *    превращал бы стол в тот самый печатный станок, против которого заводили
 *    кассу соседа: «выпало много — встал, выпало мало — ответил», и риска в
 *    игре не остаётся вовсе. Зонд ревью так и прошёл: двадцать открытий, три
 *    доигранных раунда, +600 мм при нулевом кошельке потерь. Поэтому кость,
 *    легшая на стол, — это уже сделанная ставка, и `LeaveTavernDiceRound`
 *    открыта ровно там, где ответ невозможен (`tavernRoundUnanswerableReason`).
 *
 ...
 * выхода.
 *
 * И ровно поэтому команда открыта **только** там: выход из раунда, доступный
 * всегда, — это бесплатный переброс чужой кости, потому что число соперника
 * лежит на столе до решения. Судит это `tavernRoundUnanswerableReason`, а не
 * желание игрока.
 */
...
/** Сколько медяков в кошельке героя. Пересчёт валют общий (`merchant-economy.mjs`). */
function heroPurseCp(state = {}, heroId = '') {
  const id = text(heroId, 120)
  const hero = (Array.isArray(state?.players) ? state.players : [])
    .find((player) => text(player?.id, 120) === id) ?? null
  return hero ? currencyToCopper(hero.currency) : 0
}
 ...
 * пускать ли героя из-за стола (`LeaveTavernDiceRound`, `server/rules-engine.mjs`),
 * по нему же гаснут кнопки ответа на доске и по нему же подсказка зовёт встать,
 * а не бросать. Второй ответ на этот вопрос означал бы кнопку, которая горит
 * там, где сервер откажет, — или отказ там, где кнопка погасла.
 *
 * Поводов ровно три, и все три читаются как «ответить нечем», а не «не
 * хочется»: соперник спустил всё (тогда банк ему не закрыть), свои монеты ушли
 * другой командой (тогда не закрыть ставку), героя выставили за дверь (тогда с
 * ним не садятся вовсе). Первые два — это дословно проверки
 * `AnswerTavernDiceRound`, третий — запрет входа, из-под которого ответа тоже
 * нет.
 *
 * `null` здесь — это «садись и отвечай»: кость соперника уже на столе, ставка
 * сделана, и уйти от неё нельзя.
 */
export function tavernRoundUnanswerableReason(state = {}, round = null) {
  ...
  if (heroPurseCp(state, safe.hero_id) < safe.stake_cp) return 'hero-broke'
  return null
}
...
export function tavernForViewer(state = {}, viewer = {}) {
  ...
  return {
    ...
    // ответить. Считает это сервер и одним ответом на всех — иначе кнопка
    // «встать из-за стола» горела бы там, где движок откажет.
