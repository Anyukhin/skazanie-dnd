 ...
 *    стол. Дальше она возвращается ровно тремя путями: выигрыш (ставка и приз),
 *    ничья и разоблачение шулера (ставка), уход выставленного за дверь (ставка —
 *    выставили не его волей). Проигрыш, поимка за руку и любой уход из живого
 *    раунда не возвращают ничего: уход — это сдача, и стоит он ровно столько же,
 *    сколько проигрыш.
 *
 ...
 *    команде, и автозакрытие в тот момент, когда чужая касса опустела.
 *
 *    Деньги в этих двух закрытиях расходятся, и расходятся не по факту тупика, а
 *    по его виновнику. Разорить соседа в этой игре может **только отряд**:
 *    других расходов у его кассы нет (`gamblersAfterSettlement` — единственный,
 *    кто её двигает, и двигают её события команд героев). Поэтому «соперник
 *    разорился» — всегда дело рук отряда, и ставку такое закрытие не возвращает:
 *    иначе открытие раунда получало бы положительное матожидание — «выпало
 *    хорошо — отвечаю, выпало плохо — товарищ грызёт кассу соседа, и ставка
 *    возвращается». Ревью прошло этот цикл живыми костями: заведомо проигранная
 *    ставка возвращалась целиком в 62 попытках из ста. Возврат остаётся ровно
 *    там, где тупик устроило заведение: героя выставили за дверь.
 ...
 * отличается от проигрыша — эскроу остаётся сопернику. Возврат остаётся ровно
 * одному поводу — `patron-ejected`: за дверь героя выставляет заведение, и это
 * единственный тупик, которого отряд не устраивал сам. Разорение соперника
 * ставку не возвращает, потому что разорить его может только отряд (см. пункт 7
 * заголовка модуля).
 */
 ...
 *
 * Ответ здесь один на всех, и это главное в этой функции. По нему гаснут кнопки
 * ответа на доске, по нему же подсказка зовёт встать, а не бросать, и по нему же
 * движок называет повод закрытия раунда (`LeaveTavernDiceRound`,
 * `server/rules-engine.mjs`). Второй ответ на этот вопрос означал бы горящую
 * кнопку ответа под запретом входа.
 *
 * Деньги эта функция **не решает**, и это правка ревью. Пока «тупик» и «возврат»
 * были одним словом, второй герой за тем же столом мог устроить тупик первому:
 * товарищ грыз кассу соседа, пока она не падала ниже ставки, раунд закрывался
 * сам — и заведомо проигранная ставка возвращалась целиком. Возврат зависит не
 * от факта тупика, а от того, кто его устроил, и считает это
 * `tavernRoundCancelledEvent`.
 *
 ...
 *
 * Живыми командами до этих двух состояний сегодня не добраться, и это сказано
 * прямо, чтобы следующий читатель не искал их зонда. Опустевшая касса закрывает
 * чужие раунды тем же пакетом событий, которым опустела
 * (`AnswerTavernDiceRound`), а запрет входа пишется после `TavernDiceRoundResolved`
 * того же героя — раунд к этому моменту уже снят. Функция остаётся сторожем:
 * появится второй производитель `TavernPatronEjected` (драка в зале, хозяин
 * выгнал отряд) — доска и подсказки будут готовы к нему заранее, а не после
 * того, как игрок увидит горящую кнопку под запретом.
 *
 ...
 * можно, но это сдача, и ставку он оставляет сопернику. Ставку он оставляет
 * сопернику и в первом тупике — там её тоже забрал отряд, только чужими руками.
 */
 ...
 * закрытый без броска раунд оставляет сопернику зарезервированную ставку).
 * Событие без `npc_purse_after_cp` кассу не трогает вовсе — так закрытие с
 * возвратом (героя выставили за дверь) и отличается от всех прочих.
 */
...
export function applyTavernEvent(input, event, state = {}) {
  ...
  // движок: возврат остаётся выставленному за дверь, во всех прочих закрытиях
  // ставка достаётся сопернику. Касса соседа поэтому переносится тем же путём,
