 ...
 */
const PARLEY_PHRASE = /(?:парл[еи]й|переговор|перемири|поговорим|давай(?:те)?\s+погово|погово\p{L}*\s+(?:миром|по-хорошему)|сложите\s+оружие|опустите\s+оружие|брос(?:ай|ьте)\s+оружие|прекрат\p{L}*\s+(?:бой|драк)|хватит\s+(?:драться|биться)|не\s+хотим\s+(?:драться|биться)|предлага\p{L}*\s+(?:мир|сдаться)|сдавайтесь)/iu

export function mentionsParley(value = '') {
  return PARLEY_PHRASE.test(text(value, 1_000))
}
