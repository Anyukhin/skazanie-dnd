...
function tavernHints(room) {
  ...
  if (tavern.round) {
    ...
    // положения движок и считает тупиком, поэтому повод приезжает карточкой
    // готовым словом: считать его второй раз значило бы разойтись с движком.
    //
    // Ставку из них возвращает **один** — запрет входа: его вешает заведение.
    // Разорение соседа устраивает отряд (обыграть его больше некому), и обещать
    // за него возврат подсказка не имеет права.
    ...
    return {
      ...
      urgent: [{
        ...
        text: unanswerable === 'opponent-broke'
          ? `Можно встать из-за стола: ${rival} спустил всё и банк уже не закроет — ставка в ${stake} мм со стола не вернётся`
