...
## Контракт стартовой локальной карты

Каждая запись авторского мира задаёт `opening.scene.map` как серверный рецепт
локальной тактической сцены. Обязательны `layout`, `scale`, `pattern`,
`material`, размеры и bounded-параметры воды/открытости; `theme_id` выбирает
разрешённую живую тему из каталога тактических генераторов. Для четырёх
предзаготовленных стартов `theme_id` указывает authored-вариант: гавань,
оазисную площадь, пепельную гавань или военную галерею.

Сид такой карты имеет вид `authored-scene:<template-id>@<version>` и не зависит
от кода стола или состава партии. Поэтому два запуска одного мира получают
одинаковую геометрию, материалы, воду и реквизит. Карта строится один раз при
создании кампании, сохраняется вместе с начальным состоянием и дальше живёт
через `scene.cells`/`state.locationMaps`; растровая иллюстрация остаётся только
атмосферной проекцией. Абзацы `openingNarration` параллельно становятся
подтверждёнными party-фактами с тем же canonical `location` subject; `worldHistory`
в эти стартовые факты не копируется.

В авторском дворце используются существующие одно-клеточные реквизиты стола,
светильников, колонн и server-owned люстра. Люстра имеет фиксированную высоту
и потолочную точку опоры для bounded swing ruling; `zOrder` остальных предметов
по-прежнему не является высотой, а произвольные подвешенные объекты не
поддерживаются.
