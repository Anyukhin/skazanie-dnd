...
`src/board3d-props.ts`. Immutable public release
[`bd5c4563074b6e4ef576769c`](../public/assets/models/environment/releases/bd5c4563074b6e4ef576769c/manifest.json) содержит 177 model records, 94 уникальных
mapped assetId и 11 намеренно плоских видов; это покрывает все 105 видов
реестра. В выпуске 42 авторские модели семейства `skazanie`: 12 интерьерных,
18 household и 12 settlement. Общие геометрии и материалы переиспользуются
внутри сцены.
...
Набор `actor profiles` фиксирует шесть профилей: `warrior`, `mage`, `rogue`,
`goblin`, `skeleton` и `beast`. Каталог содержит двенадцать выбираемых
записей: десять GLB и два процедурных fallback, когда отдельный файл недоступен.
Имена моделей, происхождение, высота и ограничения собраны в
[3d-models.md](3d-models.md); actor URL здесь намеренно не дублируются.
...
`public/assets/models/environment/manifest.json` указывает на immutable public
release `bd5c4563074b6e4ef576769c`, который связывает готовые GLB с
...
с файлами выпуска. `src/prop-model-catalog.ts` выбирает вариант по стабильному
`prop.id`; оба рендера получают одну и ту же модель для одного предмета.
...
силуэт. В immutable release 177 записей моделей, 94 уникальных mapped `assetId`
и 11 плоских представлений; 42 записи семейства `skazanie` — авторские,
включая 12 интерьерных, 18 household и 12 settlement. Добавление файла не
создаёт нового игрового предмета и не меняет его проходимость или взаимодействия.
...
Переходы `stairs_up` и `stairs_down` используют один self-contained GLB
`kenney-dungeon/stairs.glb` из Kenney Modular Dungeon Kit; направление остаётся
свойством перехода и поворота записи карты. Текстура `colormap.png` входит в
выпуск, поэтому загрузчику не нужен внешний ресурс.

### Кандидат и опубликованный выпуск

`models:prepare` проверяет SHA-256 двух обязательных исходных архивов по
закреплённым значениям, импортирует GLB, применяет палитру Kenney и запускает
локальный рендер атласа. Modular Dungeon подключается третьим архивом только
явными `--dungeon-dir` и `--dungeon-archive`. Отсутствующий или другой архив останавливает подготовку до
...
42 авторские модели окружения (12 интерьерных, 18 household и 12 settlement)
добавляются к ещё не опубликованному кандидату командой
`node tools/build-interior-models.mjs --dir <кандидат>`. После добавления нужно
заново отрисовать атлас и выполнить `models:check`. Столы и стойка имеют
именованный узел `surface-top`, саркофаг — отдельную петлю крышки. Подвижные
крышки и их дочерние меши исключаются из batching.
...
На Windows перед `models:publish` нужно остановить Vite и его watcher: открытый
staging-каталог может оставаться занятым, из-за чего очистка или `rename`
возвращает `EPERM`. Запись через атомарный `rename` сохраняется; обходы с
рекурсивным перемещением каталогов не используются.
...
Крышный слой строится только над раскрытыми проходимыми клетками interior-зон
и замыкается по структурным рёбрам стен, дверей, окон, бойниц и решёток.
Нераскрытая клетка не порождает геометрию и не раскрывается через крышу. Крыша
помечена как неперемещаемая и не участвует в выборе предметов.

Доступны три визуальных режима, сохраняемых локально в настройках 3D:

- `cutaway` — режим по умолчанию: сплошные скаты, своды и верхние панели стен
  скрыты, остаются карнизы, стропила и конёк, чтобы видеть игровое поле;
- `full` — показывает скаты с черепицей, верх стен и своды; обычные building и
  temple получают двускатную крышу, crypt и подземные помещения — сегментированный
  каменный свод, а естественная cave остаётся без искусственного потолка;
- `hidden` — скрывает весь крышный слой.

Режим меняет только отрисовку оболочки. Клетки, проходы, туман, высоты,
занятые площади и взаимодействия остаются данными TacticalMap и серверных
правил.
...
Набор actor profiles включает десять GLB (Quaternius и KayKit) и два
процедурных fallback; он даёт шесть профилей, но не закрывает все расы, классы
и лица. Полные ключи и происхождение перечислены в [3d-models.md](3d-models.md).
...
Крыши и своды уже работают как визуальная оболочка с режимами `cutaway`, `full`
и `hidden`; они строятся по раскрытым зонам и не являются физикой нескольких
этажей. Плавные склоны и несколько перекрывающихся этажей требуют отдельной
реализации. Видимое оружие поддержано условными комплектами;
