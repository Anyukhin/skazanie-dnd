...
- Свободное действие проходит через детерминированный fallback: короткий, мусорный или физически неподтверждённый ввод получает уточнение, а содержательный — явно ограниченное ruling-решение, которое одним идемпотентным commit обновляет цель сцены. Отдельный ручной approval-шаг этим путём не предусмотрен.
- Универсальные произвольные физические эффекты пока не исполняются: bounded ruling фиксирует только подтверждённый следующий шаг/цель, а не создаёт неподтверждённые факты о мире. Классификация свободного текста для этого пути остаётся детерминированной; неиспользуемые legacy-промпты adjudicator/intent_parser не подключены.
