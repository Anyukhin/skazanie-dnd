...
Текущий catalog покрывает малый набор предметов, EP отсутствует, а Persuasion proficiency/expertise не выводятся из полной модели персонажа. Известный `catalog_id` при `GrantItem` и других границах создания перекрывает поддельные `passive_effects`; новые специализированные эффекты (сопротивление, иммунитет, защита от крита, инициатива и damage rider) исполняются только когда полный нормализованный effect-контракт совпадает с immutable catalog-записью, а затем читаются из сохранённого экземпляра. Поэтому обычный известный предмет нельзя превратить в новый magic-item, а изменение настоящего контракта требует нового versioned `effect_id`, а не правки существующего. Исторический bounded-контракт числовых AC/save stamps для доверенных stored/homebrew экземпляров сохраняется ради обратной совместимости; player API не умеет создавать или менять такие stamps. Поле `activated` сохраняется при восстановлении доверенного snapshot, но player API не позволяет записать его иначе чем через `ActivateItem`: команда принимает только boolean-состояние, заново проверяет владельца, экипировку, настройку, активный ход и Bonus Action, а `MagicItemActivationChanged` содержит schema version и semantic request fingerprint и применяется reducer без перечитывания каталога. Viewer projection выводит статус и capabilities из каталога, но скрывает item provenance защитных эффектов противника. Девять magic-item остаются `partial` из-за честно описанных границ; произвольной компиляции catalog/homebrew-эффектов нет. Event-sourced merchant lifecycle доступен только admin/director контуру, а ShopAssembler принимает лишь allowlisted catalog stock и bounded policy. Persistent social conversation, торговля, услуги и economy clock переживают restart. Всё ещё нет signed quote expiry, theft, supply/demand, механических эффектов специализированных услуг и защиты экономики нескольких кампаний общей БД; appraisal formula и результат торга являются versioned policy кампании, а не утверждением о правиле SRD.
...
Mini-compendium содержит 50 server-owned профилей и переносит из SRD HP, КД, скорость, initiative bonus, характеристики, именные действия, ограниченные исполняемые traits и CR/XP provenance. Multiattack исполняется несколькими атаками внутри одного Attack action. Это не полные stat blocks: saves/skills, resistances/immunities, spells, большинство reactions и особых действий не исполняются. Поэтому метка сложности отражает официальный XP budget и локальную bounded allocation policy, но не доказывает фактический баланс встречи с учётом недоступной механики или состава ресурсов группы.
