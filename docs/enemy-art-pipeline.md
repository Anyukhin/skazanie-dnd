...
В наборе два поколения генераций, и это надо держать в голове при сверке
провенанса. Записи, перечисленные ниже в «Наборе промптов», созданы встроенным
ImageGen вручную — по отдельному промпту для каждого уникального вида. Всё, что
появилось после, рисует `tools/generate-enemy-portrait.mjs`: та же основа
промпта, но модель `DND_IMAGE_MODEL` через RouterAI. Инструмент сам ничего о
происхождении не записывает, поэтому у нового портрета единственный след
поколения — эта оговорка и запись в `data/asset-rights.json`.

Все 50 записей серверного бестиария имеют самостоятельные портреты: гоблин-
налётчик с коротким луком и гоблин-воин со скимитаром больше не делят один арт.
...
`tools/generate-enemy-portrait.mjs` делает шаги 2–4 из списка ниже одной
командой:

```bash
pnpm enemies:portrait --id ogre-zombie \
  --description "огр-зомби с провалившейся грудной клеткой и распухшими руками"
pnpm enemies:portrait --id ogre-zombie --description "…" --force    # перерисовать
pnpm enemies:portrait --id ogre-zombie --description "…" --dry-run  # показать промпт
```

Что он делает и чего не делает:

- зовёт картиночную модель тем же серверным путём, что и портреты NPC
  (`server/image-generation.mjs`, ключ `ROUTERAI_API_KEY` и `DND_IMAGE_MODEL`
  из `.env`), только просит `png`: `webp` в Node без новых зависимостей не
  разобрать, а привести кадр к 512 надо обязательно;
- уменьшает до 512×512 своим кодеком (`tools/png-codec.mjs`) и отвергает
  подозрительно лёгкий файл (< 20 КБ) — это тот же порог, что у сторожа;
- кладёт результат в `public/assets/enemies/<id>.png` и регистрирует хеш
  штатным `tools/register-asset-rights.mjs`;
- **не трогает** ни `SRD_5_2_1_MONSTER_ALLOWLIST`, ни темы ассемблера.
  Стат-блок с провенансом — отдельная работа, и картинка её не заменяет.

Промпт собирается из визуального контракта выше и русского описания внешности:
общая основа набора остаётся общей, а не расходится «до инструмента» и «после».

## Добавление существа

1. Добавить подтверждённый stat block в `SRD_5_2_1_MONSTER_ALLOWLIST`, не
   ослабляя ruleset и не меняя схему событий.
2. Сгенерировать оригинальный портрет: `pnpm enemies:portrait --id <id>
   --description "<внешность по-русски>"` (или вручную по визуальному
   контракту, приведя к 512×512 и сохранив как
   `public/assets/enemies/<id>.png`).
3. Указать этот путь в поле `image`.
4. Зарегистрировать права: `pnpm enemies:rights` (инструмент делает это сам).
5. Запустить `node --test test/enemy-image-assets.test.mjs`,
   `node --test test/srd-creature-expansion.test.mjs` и `pnpm content:verify`.
