...
1. Администратор выбирает в UI только сложность `easy|medium|hard` и тему `generic|goblinoids|undead|beasts`; endpoint также требует viewed state version и idempotency key. `CreateEncounter` через общий typed-command route запрещён.
...
3. `EncounterAssembler` считает официальный SRD 5.2.1 XP Budget per Character для каждого уровня 1–20 и выбирает в пределах бюджета и quantity cap одну или несколько записей из пяти server-owned профилей: Goblin Minion, Goblin Warrior, Skeleton, Zombie и Wolf. Intent агента/администратора ограничен только allowlisted difficulty/theme; числовые профили он не сочиняет.
...
Это именно mini-compendium из пяти primary-attack projections, а не полный monster corpus. В нём нет traits, saves/skills, resistances/immunities, multiattack, spells, reactions, legendary actions, tactics profile, loot или rewards. Ручной admin flow пока не подключён автоматически к Director/scene transition.
...
6. После `StartCombat` или `EndTurn` bounded scheduler последовательно обрабатывает server-owned enemies до следующего живого PC. Он детерминированно выбирает ближайшую достижимую цель, двигается/атакует/завершает ход, пропускает побеждённых и завершает бой при поражении стороны; dice при этом остаются серверными.
...
Этот цикл умеет продолжить встречу, созданную EncounterAssembler, но не является полным Tactical Controller: решения NPC пока представляют небольшую rule-based policy без LLM и используют только один основной профиль атаки.
...
| `rules-engine.mjs` | Command validation, общий lookup героев/actors/enemies, trusted combat profile, `CreateEncounter`, path/range/turn/action checks, `AdvanceScene`, appraisal/торговые события, атомарная касса и reducer/replay; авторитетен в `enforce` | Нет диагоналей, difficult terrain, cover/LOS, полной weapon/spell semantics, death saves, condition effects, concentration, bonus action/reaction и полного rest recovery |
| `encounter-assembler.mjs` | Официальный SRD 5.2.1 XP budget уровней 1–20, пять server-owned primary-attack profiles, bounded difficulty/theme, deterministic selection и безопасные revealed/reachable spawn cells не ближе 10 футов | Mini-compendium не содержит traits, saves, resistances, multiattack, spells, особые действия, tactics, loot/rewards; нет автоматического Director integration |
...
| `npc-turn-scheduler.mjs` | Bounded deterministic enemy policy, отдельные идемпотентные commits, skip defeated и auto-end до следующего PC; работает и для собранной встречи | Нет Tactical Controller agent, threat/tactics profiles, сложных legal actions, spells/reactions, loot/rewards |
...
Pack `data/rule_packs/srd_5_2_1` содержит 21 двуязычный оригинальный пересказ, 25 glossary terms, 21 ontology edge, manifest и coverage metadata. Он загружается при старте и используется `/api/rules/search` и orchestrator retrieval. Rules Engine ссылается на существующие IDs pack; известного ID mismatch нет.
