...
| Предметы | 106 записей SRD 5.2.1 | частично | да, structured command + merchant API | unit + HTTP + replay/restart | Единый immutable manifest хранит 97 позиций базового снаряжения и девять magic-item с авторитетными ценой, весом, provenance и versioned runtime-профилями. Новая волна добавляет Плащ защиты (+1 КД/спасброски, складывается с Кольцом), Брошь защиты (force resistance и иммунитет к canonical `magic-missile`), Адамантиновую кольчугу (крит становится обычным попаданием до удвоения костей), Длинный меч предупреждения (две d20 инициативы с преимуществом), Жестокий длинный меч (`2d6` рубящего, `4d6` на крите) и Огненный длинный меч (явное включение/выключение, `2d6`/`4d6` огня). `ActivateItem` проверяет владельца, экипировку, настройку, ход и Bonus Action, сохраняет schema-version/fingerprint и воспроизводится payload-only; вне боя переключение свободно. Старые replay не гидратируются задним числом, неизвестные клиентские поля и forged effects игнорируются, а проекция противника не раскрывает item provenance. Все magic-item остаются `partial`: нет полного attunement-rest, training/Stealth/don/doff, surprise у меча предупреждения, света Огненного меча, versatile/mastery и полной механики Жезла. Shop/loot/crafting разделены allowlist 12/30/0; magic-item в общую добычу не входят |
