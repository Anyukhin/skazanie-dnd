...
намерением `ignite`/`topple`/`take`, факт `scene_change` из свободного действия,
...
(`runWorldRumorClock`, `server/index.mjs`) в системном такте по мировому
времени. Задержка рождения местной молвы детерминирована тяжестью (6/12/24
часа), каждый шаг по графу мира стоит суток, дальше двух шагов слух не идёт, а
формулировка деградирует по трём ступеням точности. Слух прямого свидетеля
подтверждается провенансом поступка, пересказ остаётся `unknown`. Реакция мира —
существующий `FactionReputationAdjusted`, один раз на пару «поступок + фракция»,
с поправкой, слабеющей по дальности.

Проверено `test/world-deeds.test.mjs`: тайна без свидетелей, рождение молвы по
часам, география расползания, деградация формулировки, влияние на репутацию и
его однократность, replay без удвоения, чистота gm_only в проекции игрока и
попадание дошедшего слуха в разговор NPC. Не закрыто: ложь и взлом двери
поступком не считаются, у пропса нет хозяина (кража опознаётся по взятому на
людях), плена в движке нет — поэтому «жестокость к пленному» из брифа осталась
