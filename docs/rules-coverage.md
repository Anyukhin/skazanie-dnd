...
|---|---|---|---|---|---|
| Перемещение | нет | частично | да, enforce UI/API | unit + HTTP | Ортогональный shortest path, стены, occupancy, speed/action economy, выход из 5-футовой зоны досягаемости и server-owned труднопроходимые active effects проверяются; клиент показывает двойную стоимость и штриховку. Нет диагоналей, forced movement; статический `moveCost` authored-карты пока влияет только на клиентский маршрут и нуждается в отдельной серверной команде/проверке |
