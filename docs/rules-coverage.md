...
| Предметы | 97 записей SRD 5.2.1 | частично | да, structured command + merchant API | unit + HTTP + replay | Единый immutable manifest хранит 38 видов оружия, 13 доспехов/щитов, 5 комплектов боеприпасов, 17 ремесленных инструментов, набор травника и 23 практических предмета с ценой, весом, provenance, runtime-профилями и `mechanics_status`. При создании экземпляра сервер материализует авторитетные поля; старые события replay не гидратируются задним числом. Боевая атака подключена только у прежних 4 видов оружия, КД — у доспехов/щитов, use — у зелья и пайка; mastery, расход ammunition, versatile, training, Stealth, don/doff, общий crafting и большинство Utilize-эффектов не исполнены. Shop/loot/crafting разделены явными allowlist 12/9/0 |
