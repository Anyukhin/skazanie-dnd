...
| Предметы | 98 записей SRD 5.2.1 | частично | да, structured command + merchant API | unit + HTTP + replay | Единый immutable manifest хранит 97 позиций базового снаряжения и «Длинный меч +1» с ценой, весом, provenance, runtime-профилями и `mechanics_status`. При создании экземпляра сервер материализует авторитетные поля; старые события replay не гидратируются задним числом, а UI-возможности безопасно выводятся проекцией. Зелье лечения (`2d4+2`, Bonus Action, живая party-цель в 5 футах) и набор лекаря (10 применений, Action, стабилизация героя с 0 ОЗ без броска) имеют `verified`; заряды набора сохраняются `ItemChargesSpent` в том же атомарном commit. Длинный меч +1 не требует настройки и серверно добавляет +1 к атаке и урону, но остаётся `partial` из-за mastery Sap и versatile. Mastery, расход ammunition, training, Stealth, don/doff, общий crafting и большинство Utilize-эффектов не исполнены. Shop/loot/crafting разделены явными allowlist 12/9/0; magic-item в них не входит |
