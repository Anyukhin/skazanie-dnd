...
| Объекты сцены | server-owned catalog | да для `inspect/open/take/use` | да, enforce UI/API + free text | unit + HTTP + replay/reopen | `OperateSceneObject` обслуживает сундук/бочку/ящик, алтарь/руну/статую, костёр, полку/стол и останки одним путём: соседство, закрытый замок и ловушка, серверный d20/СЛ, действие в бою, короткий отдых, bounded-знание/эффект и добыча из общей таблицы. Кнопка и свободный текст сходятся в этой команде; 2–4 POI выбираются по seed, скрытые ключи срезает player/Narrator projection. Нет разрушаемого terrain state, полного набора ловушек/замков и расширенного каталога находок |
