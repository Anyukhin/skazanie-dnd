...
| Предметы | 99 записей SRD 5.2.1 | частично | да, structured command + merchant API | unit + HTTP + replay/restart | Единый immutable manifest хранит 97 позиций базового снаряжения, «Длинный меч +1» и «Кольцо защиты» с ценой, весом, provenance, runtime-профилями и `mechanics_status`. При создании экземпляра сервер материализует авторитетные поля и versioned `passive_effects`; старые события replay без stamp не гидратируются задним числом, а UI-возможности, статус и ограничение безопасно выводятся проекцией. Зелье лечения (`2d4+2`, Bonus Action, живая party-цель в 5 футах) и набор лекаря (10 применений, Action, стабилизация героя с 0 ОЗ без броска) имеют `verified`; заряды и настройка сохраняются событиями. Числовой срез Кольца защиты проверен: экипированное и настроенное кольцо даёт +1 к КД и всем спасброскам, одинаковые копии не складываются. Общий статус кольца — `partial`, потому что lifecycle настройки пока не моделирует отдельный Short Rest; карточка показывает это ограничение. Длинный меч +1 не требует настройки и серверно добавляет +1 к атаке и урону, но остаётся `partial` из-за mastery Sap и versatile. Есть payload-only foundation перезарядки `1d6+1` на рассвете: оба существующих producer-path времени считают границы арифметически, бросают до commit и сохраняют один bounded aggregate event; viewer видит current/max/recharge без roll payload. Ни одна из 99 каталожных позиций пока не включает этот профиль, Wand of Magic Missiles не реализована. Mastery, расход ammunition, training, Stealth, don/doff, общий crafting и большинство Utilize-эффектов не исполнены. Shop/loot/crafting разделены явными allowlist 12/9/0; magic-item в них не входит |
