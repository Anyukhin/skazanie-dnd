...
**Профиль.** У безымянного врага появляется социальный профиль: имя (прозвище +
имя) и характер выводятся детерминированно из `sessionCode` и id актора, поэтому
replay даёт того же человека. Уже существующий профиль не переписывается. Пленный
...
(`server/world-deeds.mjs`); смерть пленного не считается ещё и убийством, чтобы
один нож не записался дважды. Голод отмечают серверные часы (`runCaptiveClock`,
`server/index.mjs`) через сутки без еды; кнопки «объявить голод» у отряда нет.
