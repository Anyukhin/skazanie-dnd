...
test('падение до нуля HP и проверка концентрации явно представлены событиями', () => {
  let state = stateFixture()
  state.mechanics.concentration.hero = { effect_id: 'spell:web' }
  state.mechanics.temporary_hp.hero = 0
  const result = resolveCommand({ command_type: 'ApplyDamage', actor_id: 'goblin', target_id: 'hero', amount: 20, damage_type: 'cold' }, state, { diceService: dice([]) })
  assert.deepEqual(result.events.map((event) => event.event_type), ['DamageApplied', 'HitPointsReducedToZero', 'ConcentrationCheckRequired'])
  const after = result.events.reduce(applyGameEvent, state)
