...
function fixture({ dying = false, paladinConditions = [], includeWeakPaladin = true } = {}) {
  ...
  return normalizeCampaignState({
    ...
    players: [
      ...
      { id: 'ally', character: 'Торн', characterClass: 'fighter', level: 6, hp: dying ? 0 : 32, maxHp: 32, armor: 17, speed: 30, proficiency: 3, abilities: { str: 16, dex: 10, con: 14, int: 10, wis: 10, cha: 10 }, x: 1, y: 2, inventory: [] },
...
test('Аура защиты применяется к death save, но не отменяет особый эффект натуральной 1', () => {
  const state = fixture({ dying: true, includeWeakPaladin: false })
  ...
  assert.equal(saveEvent.payload.total, 10)
  ...
  assert.equal(oneEvent.payload.total, 5)
...
test('Заклинательный save использует ауру союзного паладина цели', () => {
  ...
  state.enemies = [
    { id: 'enemy', name: 'Страж', characterClass: 'fighter', level: 6, hp: 24, maxHp: 24, armor: 12, speed: 30, abilities: { str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10 }, x: 2, y: 1, alive: true },
  ...
  assert.equal(save.payload.total, 14)
