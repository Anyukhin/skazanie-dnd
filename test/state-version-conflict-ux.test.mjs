...
test('свободный и подготовленный ввод очищаются только после успешной команды', () => {
  ...
  assert.match(appSource, /if \(outcome\?\.ok\) \{\s+if \(pendingCommand\.kind === 'target' && pendingCommand\.sneakAttack\) setSneakAttack\(false\)\s+setPendingCommand\(null\)\s+setSpellTargetIds\(\[\]\)\s+setAreaSpellPoint\(null\)\s+setAimCell\(null\)\s+\}/u)
