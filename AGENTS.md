...
- **Никогда не читать, не печатать и не коммитить `.env`.** Он в `.gitignore`.
  Новые переменные добавлять только в `.env.example` с комментарием и без значения.
- Ключ LLM (`ROUTERAI_API_KEY`) и токены не должны попадать в логи, тесты,
  сообщения коммитов и текст ответов.
